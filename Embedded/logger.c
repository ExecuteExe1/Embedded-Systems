...
{
    FILE *f = fopen("/proc/stat", "r");/*Open file if it fails return -1*/
    if (!f) return -1;

    char label[16];
/*read numbers,store them into out*/
    ...
    fclose(f); /*close file*/
    return (n == 9) ? 0 : -1; /*If all 9 calues were read return 0 otherwise -1*/
}
...
static double cpu_pct_from_delta(const cpu_jiffies_t *prev, const cpu_jiffies_t *cur)
{
    ...
    double busy_pct = 100.0 * (delta_total - delta_idle) / delta_total; /*CPU usage*/
    if (busy_pct < 0.0) busy_pct = 0.0; /*Clamp keeps the values valid,sometimes they may produce CPU usage 100%*/
...
Fires exactly once per second, aligned to the wall-clock second boundary, using clock_nanosleep(CLOCK_REALTIME, TIMER_ABSTIME, ...).Using an ABSOLUTE target that is incremented by a fixed 1s each iteration (rather than repeatedly sleeping "1 second from now")is what prevents clock drift over a 24h run: any scheduling delay in one iteration does not push subsequent wakeups later, because the target times are computed independently of when we actually woke.
 */
void *logger_thread_main(void *arg)
{
    (void)arg;

    FILE *log = fopen(LOG_FILE_PATH, "a");/*append mode,existing data remains,new data is added*/
    if (!log) {
        fprintf(stderr, "logger: could not open %s for append\n", LOG_FILE_PATH);
        return NULL; /*if opening failsthread terminated*/
    }
    ...
    while (g_state.running) {
        ...
        /* Seconds,Nanoseconds,Commit_Count,Identity_Count,Account_Count,
           Info_Count,Buffer_Occupancy_Pct,CPU_Pct
            Message counts become "NaN" (not 0) whenever the link was down at any point in the window just measured, so a silent second caused by an idle network can never be confused, during  post-processing, with a silent second caused by being offline. */
